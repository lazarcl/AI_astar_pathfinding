...
Version: 3.7 (last updated January, 2014)
...
This has been tested with Python 2.7.6 and Pygame 1.9.1/2.
"""

print ("using graphics.py library version 3.7")
...
def drawPoint(x, y, color=_GLI.foreground):
...
def onKeyPress(listenerFunction, key=None):
    key = getKeyCode(key)
    if key == None:
...
def onKeyRelease(listenerFunction, key=None):
    key = getKeyCode(key)
    if key == None:
                ...
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_ESCAPE:
                        ...
                        if ("keydown",event.key) in _GLI.eventListeners:
                            _GLI.eventListeners[("keydown",event.key)](_GLI.world, event.key)
                        else:
                    ...
                    if ("keyup",event.key) in _GLI.eventListeners:
                        _GLI.eventListeners[("keyup",event.key)](_GLI.world, event.key)
                    else:
